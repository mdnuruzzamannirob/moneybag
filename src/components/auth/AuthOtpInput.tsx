...
export function AuthOtpInput({
  ...
  const fillFrom = (rawValue: string, startIndex: number) => {
    const pastedDigits = rawValue.replace(/\D/g, '').slice(0, OTP_LENGTH - startIndex);
    if (!pastedDigits) return;
    const nextDigits = [...digits];
    pastedDigits.split('').forEach((digit, offset) => {
      nextDigits[startIndex + offset] = digit;
    });
    setDigits(nextDigits, Math.min(startIndex + pastedDigits.length, OTP_LENGTH - 1));
  };
  ...
  return (
    <div className="flex w-full items-center justify-center gap-1.5 xs:gap-2 sm:gap-3" role="group">
      {digits.map((digit, index) => (
        <input
          ...
          onChange={(event) => {
            ...
            setDigits(nextDigits, nextValue ? Math.min(index + 1, OTP_LENGTH - 1) : undefined);
          }}
