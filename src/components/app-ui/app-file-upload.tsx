...
export function AppFileUpload({
  ...
  return (
    <div className="rounded-lg border border-dashed border-border px-4 py-6 transition-colors hover:bg-muted/40 sm:px-6 sm:py-8">
      ...
      {files.length ? (
        <div className="mt-5 space-y-2 border-t border-border pt-4">
          {files.map((file) => (
            <div
              className="flex min-w-0 items-center gap-2 rounded-md border border-border bg-card px-3 py-2 text-xs shadow-xs"
              key={file}
            >
